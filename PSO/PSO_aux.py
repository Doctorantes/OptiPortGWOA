...
G.add_edges_from([("a", "b") ,('c','b'),("a", "b") ])

#print(nx.check_planarity(G))

#G.add_edge(("a", "b") )
#G=nx.contracted_nodes(G, 'el','b')
G.add_edge('a','a')
#print(G.edges)
#nx.write_graphml(G, "prueba.graphml")
#print(G.nodes)
#print(list(G.neighbors('a')),'pppp')
G2 = nx.read_graphml('prueba.graphml')
#print(G2.nodes)
#print(G2.edges)
G=G2
...
x = list(map(tuple, x))
y=np.array([5,4,3])
#print(set([5,4,2])==set([2,4,5]))
#print((np.array([False,True]) | np.array([True, False])))
#x[(5,4)]=8

#print(list(range(50,300,50)))
...
#optimizer = ps.single.GlobalBestPSO(n_particles=200, dimensions=1000, options=options)
# Perform optimization
#best_cost, best_pos = optimizer.optimize(fx.sphere, iters=2000)
...
options = {'c1': 2, 'c2': 2, 'w':0.3}
# Call instance of PSO
optimizer = ps.single.GlobalBestPSO(n_particles=50, dimensions=30, options=options)
# Perform optimization
best_cost, best_pos = optimizer.optimize(fx.sphere, iters=1000)
#print(np.sum(np.array([[5,5,5]])**2))
#print(fx.sphere(np.array([[12,40,50]])))
#print(F1(np.array([12,40,50])))
...
def prueba():
	for i in Functions.keys():
		...
		#llaves.append(i)
		print(r)
		optimizer = ps.single.GlobalBestPSO(n_particles=50, dimensions=r[3], options=options)
		best_cost, best_pos = optimizer.optimize(r[0], iters=1000)
